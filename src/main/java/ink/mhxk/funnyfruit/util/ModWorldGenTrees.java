package ink.mhxk.funnyfruit.util;
...
extends WorldGenAbstractTree{
	 private static final IBlockState DEFAULT_TRUNK = Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.OAK);
	    ...
	    private final IBlockState fruitLeves;
	    private final Block blockFruitLevels;
	    public ModWorldGenTrees(boolean notify, int minTreeHeightIn, IBlockState woodMeta, IBlockState p_i46446_4_,IBlockState fruitLeves, boolean growVines)
	    {
	        super(notify);
	        this.minTreeHeight = minTreeHeightIn;
	        this.metaWood = woodMeta;
	        this.metaLeaves = p_i46446_4_;
	        this.fruitLeves = fruitLeves;
	        this.vinesGrow = growVines;
	       this.blockFruitLevels = fruitLeves.getBlock();
	    }
	                        ...
	                        for (int k1 = position.getX() - j1; k1 <= position.getX() + j1; ++k1)
	                        {
	                            ...
	                            for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2)
	                            {
	                                ...
	                                if (Math.abs(l1) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0)
	                                {
	                                    ...
	                                    {
	                                    	if(rand.nextInt(4)==0&&
	                                    			worldIn.getBlockState(blockpos.up()).getBlock()!=this.blockFruitLevels
	                                    			&&worldIn.getBlockState(blockpos.down()).getBlock()!=this.blockFruitLevels
	                                    			&&worldIn.getBlockState(blockpos.west()).getBlock()!=this.blockFruitLevels
	                                    			&&worldIn.getBlockState(blockpos.east()).getBlock()!=this.blockFruitLevels
	                                    			&&worldIn.getBlockState(blockpos.south()).getBlock()!=this.blockFruitLevels
	                                    			&&worldIn.getBlockState(blockpos.north()).getBlock()!=this.blockFruitLevels)
	                                        this.setBlockAndNotifyAdequately(worldIn, blockpos, this.fruitLeves);
	                                    	else
